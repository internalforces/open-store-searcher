...
|---|---|---|---|
| DEBT-002 | ADR-009 accepts the zero-key Seoul ZIP candidate, but permission coverage for all selected categories, automation stability, complete schema, cross-entry timestamp consistency, and the as-of rule remain unverified | Collector implementation and production collection remain prohibited; failed assumptions could publish incomplete or misleading data | M1 / TASK-004 permission gate, then TASK-005 and later owning tasks |
